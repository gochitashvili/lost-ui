...
export default function Stats01() {
  return (
    <div className="mx-auto grid grid-cols-1 gap-px bg-border sm:grid-cols-2 lg:grid-cols-4">
      {data.map((stat) => (
        <Card key={stat.name} className="rounded-none border-0 shadow-none py-0">
          <CardContent className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2 p-4 sm:p-6 xl:p-8">
